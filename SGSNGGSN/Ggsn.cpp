...
void sendPdpContextAccept(SgsnInfo *si, PdpContext *pdp)
{
	...
	setPco(pdpa.mPco,pdp->mPcoReq, pdp->mgp);
...
#if RN_UMTS
                pdp = new PdpContext(gmm,mgp,pdpr.mNSapi,0);
#else
		pdp = new PdpContext(gmm,mgp,pdpr.mNSapi,pdpr.mLlcSapi);
#endif
		// Update the transaction id now now before we send the UMTS RadioBearerSetup because
		// we will need the transaction id to detect a dup request if one comes in while the
		// RadioBearerSetup is in progress.
		pdp->update(pdpr);
...
void Ggsn::handleL3SmMsg(SgsnInfo *si,L3GprsFrame &frame1)
{
	...
	LOGWATCH("UL SM Msg: "<< L3SmMsg::name(mt,false) << si);
	switch (mt) {
