/**@file @brief Common elements for L3 messages, GSM 04.08 10.5.1.  */
...
class L3CellIdentity : public L3ProtocolElement {
	...
	L3CellIdentity(unsigned wID=gConfig.getNum("UMTS.Identity.CI"))
		:mID(wID)
	{ }
...
class L3LocationAreaIdentity : public L3ProtocolElement {
	...
	*/
	L3LocationAreaIdentity(
		const char*wMCC = gConfig.getStr("UMTS.Identity.MCC").c_str(),
		const char* wMNC = gConfig.getStr("UMTS.Identity.MNC").c_str(),
		unsigned wLAC = gConfig.getNum("UMTS.Identity.LAC")
	);
