...
class L2Address {
	...
	// (pat) The name CR conflicts with some header file somewhere.
	// We should rename all these short capitalized accessors.
	unsigned getCR() const { return mCR; }
...
class L2Frame : public BitVector {
	...
	*/
	...
	bool getCR() const { return mStart[6] & 0x01; }
...
// Pat added class to insure that L3Frame does not have a memory leak.
DEFINE_MEMORY_LEAK_DETECTOR_CLASS(L3Frame,MemCheckL3Frame)
...
class L3Frame : public BitVector, public MemCheckL3Frame {
