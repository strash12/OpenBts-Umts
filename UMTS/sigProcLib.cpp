...
void initGMSKRotationTables(int samplesPerSymbol) {
  ...
  RN_MEMLOG(signalVector,GMSKRotation);
  GMSKReverseRotation = new signalVector(157*samplesPerSymbol);
  RN_MEMLOG(signalVector,GMSKReverseRotation);
...
void initSincTables(void) {
  for (int i = 0; i < SINCTABLESIZE; i++) {
    ...
    RN_MEMLOG(signalVector,sincTable[i]);
			     ...
			     float *finalPhase)
{
  ...
  if (y==NULL) {
	...
	RN_MEMLOG(signalVector,y);
    y->isRealOnly(x->isRealOnly());
    if (y==NULL) return NULL;	// (pat) say, what?
  }
  ...
  *TOA = (*TOA) - (maxTOA); 
  ...
  if (requestChannel && (peakToMean > detectThreshold)) {
    ...
    *channelResponse = new signalVector(channelVector.size());
    RN_MEMLOG(signalVector,*channelResponse);
			     ...
			     int decimationFactor) 
{
  ...
  RN_MEMLOG(signalVector,decVector);
...

{
  ...
  RN_MEMLOG(signalVector,resampledVector);
	       ...
	       signalVector **feedbackFilter)
{
  ...
  *G0ptr = 1.0/sqrtf(SNRestimate);
  ...
  float d = 0;	// (pat) unused init to shut up gcc
  for(int i = 0; i < Nf; i++) {
    d = G0.begin()->norm2() + G1.begin()->norm2();
    L[i] = new signalVector(Nf+nu);
    RN_MEMLOG(signalVector,L[i]);
    Lptr = L[i]->begin()+i;
  ...
  *feedbackFilter = new signalVector(nu);
  RN_MEMLOG(signalVector,*feedbackFilter);
  ...
  *feedForwardFilter = new signalVector(Nf);
  RN_MEMLOG(signalVector,*feedForwardFilter);
		       ...
		       signalVector &b) // feedback filter
{
  ...
  RN_MEMLOG(signalVector,postForward);
  ...
  RN_MEMLOG(signalVector,DFEoutput);
